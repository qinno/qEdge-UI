...
int16_t sViewGetTouchField() {
  ...
  while (tft.getTouch(&xt, &yt, TFT_THRESH))
    ;  // wait until released
  xw     = tft.width();
  yw     = tft.height();
  dpress = millis() - dt;
...
/*
* copy file from SD to LittleFS, same name
*/
bool sViewCopySDtoLFS(String fileName) {

  File source, dest;
  source = SD.open(fileName, "r");
  if (!source) {
    Dln("SD missing " + fileName);
    return (false);
  }
  dest = LFS.open(fileName, "w");
  if (!dest) {
    Dln("LFS error " + fileName);
    return (false);
  }
...
void sViewInit() {
  ...
  if (LFSpresent and SDpresent) {            // only if both drives are present
    if (!LittleFS.exists(TFT_PNG_BOOT)) {    //check png on LittleFS
      if (sViewCopySDtoLFS(TFT_PNG_BOOT)) {  // try copy from SD
        rebootRequired = true;               // if successfull force reboot
      }
    }
    if (!LittleFS.exists(TFT_PNG_MENU)) {
      if (sViewCopySDtoLFS(TFT_PNG_MENU)) {
        rebootRequired = true;
      }
    }
    if (!LittleFS.exists(TFT_PNG_WALL)) {
      if (sViewCopySDtoLFS(TFT_PNG_WALL)) {
        rebootRequired = true;
      }
    }
    if (!LittleFS.exists(TFT_PNG_IDLE)) {
      if (sViewCopySDtoLFS(TFT_PNG_IDLE)) {
        rebootRequired = true;
      }
    }
  }
  if (rebootRequired) rp2040.reboot();


  wifiConnect();  // start WiFi
  ntpConnect();   // start NTP
  Dln("SmartView Library done");
}
